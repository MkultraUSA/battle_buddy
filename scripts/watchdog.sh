...
CALLS_1H=$(curl -s http://localhost:9001/metrics | awk -F'{' '/battlebuddy_transcript_quality_calls{window="1h"}/ {print $2}' | cut -d'}' -f1 | tr -d ' ')
...
CALLS_15=$(curl -s http://localhost:9001/metrics | awk -F'{' '/battlebuddy_transcript_quality_calls{window="15m"}/ {print $2}' | cut -d'}' -f1 | tr -d ' ')
if [ -z "$CALLS_15" ] || [ "$CALLS_15" -eq 0 ]; then
    :   # no calls in the last 15 min – skip quality checks
else
    COVERAGE=$(curl -s http://localhost:9001/metrics | awk -F'{' '/battlebuddy_transcript_quality_coverage_ratio{window="15m"}/ {print $2}' | cut -d'}' -f1 | tr -d ' ')
    RELIAB=$(curl -s http://localhost:9001/metrics | awk -F'{' '/battlebuddy_transcript_quality_reliability_score{window="15m"}/ {print $2}' | cut -d'}' -f1 | tr -d ' ')
