...
Battle Buddy listens for the phrase **"Hey Battle Buddy"** using the local faster-whisper tiny model. No cloud service or API key is required for wake word detection.

### How it works
1. Say **"Hey Battle Buddy"** — the system plays a two-tone chime to confirm it heard you
2. The current 4-hour sitrep is generated by Claude and spoken aloud by Piper TTS
3. The sitrep is also displayed on the heads-up display
...
- Mic input volume is set to 85% automatically on each service start
