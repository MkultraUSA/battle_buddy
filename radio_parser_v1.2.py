#!/usr/bin/env python3
"""
Battle Buddy — Radio Log → Incident Parser  v1.2
...
Usage:
    cd ~/battle_buddy
    python3 radio_parser_v1.2.py --log logs/radio_law_20260306.log
    python3 radio_parser_v1.2.py --log logs/radio_law_20260306.log --push
    python3 radio_parser_v1.2.py --log logs/radio_law_20260306.log --push --verify
    python3 radio_parser_v1.2.py --log logs/radio_law_20260306.log --reset --push
...
PHONETRACK_BASE = "https://kevcloud.ddns.net/apps/phonetrack/logGet"
PHONETRACK_TOKEN = "23b91519e13f254f4fecb9a6802f6cc5"

# PhoneTrack REST API — used by --verify to confirm the point landed
# GET /apps/phonetrack/api/getlastpositions/{session_token}/{device_name}
PHONETRACK_API_BASE = "https://kevcloud.ddns.net/apps/phonetrack/api/getlastpositions"
...
def verify_last_position(device: str, expected_ts: int) -> bool:
    ...
    try:
        with urllib.request.urlopen(req, timeout=10) as resp:
            data = json.loads(resp.read())
        # Response structure: { "devicename": { "lat": ..., "lon": ..., "timestamp": ... } }
...
def main():
    p = argparse.ArgumentParser(
        description=(
            ...
            f"(Ollama llama3 + Nominatim/Travis County + PhoneTrack logGet)"
        )
    )
