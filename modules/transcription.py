...
_MAX_PROCESS_THREADS = 3
_BROADCASTIFY_MAX    = 2
_process_sem         = threading.Semaphore(_MAX_PROCESS_THREADS)
_broadcastify_sem    = threading.Semaphore(_BROADCASTIFY_MAX)

# Lock reset tracking — when a transcription thread hangs, we replace the
# model lock so new calls are not blocked.  The abandoned daemon thread still
# exists but its lock reference is orphaned and it cannot deadlock fresh calls.
_lock_reset_count = 0
_lock_reset_lock = threading.Lock()
...
# How long to wait for the model lock before forcing a reset (shorter than
# the transcription timeout so we reset the lock before the caller gives up).
_MODEL_LOCK_TIMEOUT = 60


def _reset_model_lock() -> None:
    """Replace the model lock with a fresh one.

    Called when a transcription thread is suspected hung and we need to
    unblock subsequent callers.  The old lock is orphaned; the hung thread
    (if it ever finishes) will release a lock nobody is waiting on.
    """
    global _fw_model_lock, _lock_reset_count
    with _lock_reset_lock:
        _lock_reset_count += 1
        _fw_model_lock = threading.Lock()
        print(
            f"[whisper] LOCK RESET #{_lock_reset_count} — "
            f"orphaned previous lock to unblock new transcriptions",
            flush=True,
        )
...
def transcribe_with_timeout(wav_bytes: bytes, timeout: int = TRANSCRIPTION_TIMEOUT) -> str:
    ...
    def _worker() -> None:
        with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as f:
            ...
            tmp = f.name
        try:
            acquired = _fw_model_lock.acquire(timeout=_MODEL_LOCK_TIMEOUT)
            if not acquired:
                print("[whisper] TIMEOUT waiting for model lock — resetting lock", flush=True)
                _reset_model_lock()
                _fw_model_lock.acquire()
    ...
    if thread.is_alive():
        print(
            f"[whisper] transcription thread hung after {timeout}s — "
            f"abandoning thread + resetting model lock",
            flush=True,
        )
        _reset_model_lock()
        _watchdog.reset()
